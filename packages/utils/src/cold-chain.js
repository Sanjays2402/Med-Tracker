...
exports.temperatureDerating = temperatureDerating;
exports.computeColdChainStatus = computeColdChainStatus;
 ...
 */
function temperatureDerating(observedC, nominalC) {
    if (observedC <= nominalC)
        return 1;
    const delta = observedC - nominalC;
    return Math.pow(2, delta / 5);
}
function computeColdChainStatus(input) {
    ...
    if (Number.isNaN(firstUseMs))
        throw new Error('firstUseAt is not a valid datetime');
    if (Number.isNaN(nowMs))
        throw new Error('now is not a valid datetime');
    if (nowMs < firstUseMs)
        throw new Error('now must be at or after firstUseAt');
    ...
    if (input.spec.manufacturerExpiresAt) {
        ...
        if (!Number.isNaN(mfgMs)) {
            if (mfgMs < discardByMs)
                discardByMs = mfgMs;
            if (mfgMs <= nowMs)
                expiredByManufacturer = true;
        }
    }
    let status = 'ok';
