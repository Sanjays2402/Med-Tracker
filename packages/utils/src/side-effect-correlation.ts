...
export interface DoseEvent {
  medicationId: string;
  /** UTC ISO time the dose was actually taken. */
  takenAt: string;
}

export interface SymptomEvent {
  /** Free-form symptom label, normalized to lowercase by the caller. */
  symptom: string;
  /** UTC ISO time the patient reported onset. */
  reportedAt: string;
  /** Severity 1..10 if known. */
  severity?: number;
}
...
export interface CorrelationInput {
  ...
  /** Medication start dates, keyed by medicationId (UTC ISO). */
  medicationStarts: Record<string, string>;
  /** Window after a dose, in hours, used for onset coupling. Default 6. */
  windowHours?: number;
  /** Minimum dose count to evaluate a medication. Default 5. */
  minDoses?: number;
  /** Minimum symptom count to evaluate a symptom. Default 3. */
  minSymptoms?: number;
}
...
export interface CorrelationFinding {
  medicationId: string;
  symptom: string;
  /** Symptom reports falling within windowHours after a dose. */
  inWindowReports: number;
  /** Total symptom reports during the observation period. */
  totalReports: number;
  /** Fraction of observation hours that fall inside any dose window. */
  windowCoverage: number;
  /** Expected in-window reports if symptoms were uniformly distributed. */
  expectedInWindow: number;
  /** Symptom reports per day before med introduction. */
  baselinePerDay: number;
  /** Symptom reports per day after med introduction. */
  postPerDay: number;
  /** Score in [0,1]. Higher means stronger apparent association. */
  score: number;
  reason: string;
}
...
export interface CorrelationReport {
  ...
  /** Symptoms with too few reports to evaluate. */
  ignoredSymptoms: string[];
  /** Medications with too few doses to evaluate. */
  ignoredMedications: string[];
}

const HOUR_MS = 3_600_000;
const DAY_MS = 86_400_000;

export function correlateSideEffects(input: CorrelationInput): CorrelationReport {
  const windowH = input.windowHours ?? 6;
  const minDoses = input.minDoses ?? 5;
  const minSymptoms = input.minSymptoms ?? 3;

  const dosesByMed = new Map<string, number[]>();
  ...
  for (const arr of dosesByMed.values()) arr.sort((a, b) => a - b);

  const symptomsByLabel = new Map<string, number[]>();
  ...
  for (const arr of symptomsByLabel.values()) arr.sort((a, b) => a - b);

  // Observation window is min..max across all events.
  const allTimes: number[] = [];
  for (const arr of dosesByMed.values()) allTimes.push(...arr);
  for (const arr of symptomsByLabel.values()) allTimes.push(...arr);
  ...
  const ignoredSymptoms: string[] = [];
  const ignoredMedications: string[] = [];

  for (const [sym, arr] of symptomsByLabel) {
    if (arr.length < minSymptoms) ignoredSymptoms.push(sym);
  }
  for (const [med, arr] of dosesByMed) {
    if (arr.length < minDoses) ignoredMedications.push(med);
  }

  const findings: CorrelationFinding[] = [];

  for (const [med, doseTimes] of dosesByMed) {
    if (doseTimes.length < minDoses) continue;
    const windows = mergeWindows(doseTimes.map((t) => [t, t + windowH * HOUR_MS] as [number, number]));
    ...
    for (const [sym, sympTimes] of symptomsByLabel) {
      if (sympTimes.length < minSymptoms) continue;
...
function insideAny(t: number, windows: [number, number][]): boolean {
...
function buildReason(score: number, inWindow: number, expected: number, baseline: number, post: number): string {
  const parts: string[] = [];
  ...
  if (parts.length === 0) return score > 0 ? 'weak signal worth monitoring' : 'no apparent association';
  return parts.join('; ');
}
