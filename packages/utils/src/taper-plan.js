...
exports.generateTaperPlan = generateTaperPlan;
/**
 ...
 */
function decompose(targetMg, strengths, tolerance) {
    if (targetMg <= tolerance)
        return [];
...
function sumDose(units) {
    return units.reduce((s, u) => s + u.strengthMg * u.count, 0);
}
function generateTaperPlan(req) {
    if (req.startDoseMg <= 0)
        throw new Error('startDoseMg must be positive');
    if (req.endDoseMg < 0)
        throw new Error('endDoseMg must be non-negative');
    if (req.endDoseMg > req.startDoseMg)
        throw new Error('endDoseMg must be <= startDoseMg');
    if (req.allowedStrengthsMg.length === 0)
        throw new Error('allowedStrengthsMg required');
    if (req.shape === 'linear' && req.stepSize <= 0)
        throw new Error('linear stepSize must be > 0');
    ...
    while (current > req.endDoseMg + tolerance && step < maxSteps) {
        step += 1;
        let target = req.shape === 'linear' ? current - req.stepSize : current * (1 - req.stepSize);
        if (target < req.endDoseMg)
        ...
        const isHold = req.holdEveryNSteps !== undefined && step % req.holdEveryNSteps === 0;
        phases.push({
            step,
            targetDoseMg: round3(target),
            actualDoseMg: round3(actual),
            units,
            durationDays: isHold ? req.holdDurationDays : req.stepDurationDays,
            hold: isHold,
        });
        current = actual;
    }
