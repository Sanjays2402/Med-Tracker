...
/**
 * Rule-based interaction severity classifier.
 *
 * Severity ladder: contraindicated > major > moderate > minor.
 * Rules combine known drug-class pairings, individual drug warnings, and
 * interaction-keyword overlap. Each result includes a mechanism description and
 * a recommended action so consumers (UI, caregiver views, exports) can render
 * useful guidance instead of a generic "possible interaction" string.
 */

export type SeverityLevel = 'minor' | 'moderate' | 'major' | 'contraindicated';

export interface SeverityRule {
  /** Lowercased class or generic substrings matched against drug A. */
  a: string[];
  /** Lowercased class or generic substrings matched against drug B. */
  b: string[];
...
function pairMatches(a: Drug, b: Drug, rule: SeverityRule): boolean {
  return (
    (matchesSide(a, rule.a) && matchesSide(b, rule.b)) ||
    (matchesSide(a, rule.b) && matchesSide(b, rule.a))
  );
}

/** Severity inferred when only a keyword overlap exists, no curated rule fires. */
function keywordSeverity(a: Drug, b: Drug): { severity: SeverityLevel; matched: boolean } {
  ...
  if (!overlap) return { severity: 'minor', matched: false };
  ...
  if (/avoid|major|severe|bleed/.test(warn)) return { severity: 'major', matched: true };
 ...
 */
export function classifyInteractions(active: Drug[]): ScoredInteraction[] {
  ...
  for (let i = 0; i < active.length; i++) {
    for (let j = i + 1; j < active.length; j++) {
      ...
      let chosen: { rule: SeverityRule; rank: number } | null = null;
      for (const rule of SEVERITY_RULES) {
        if (!pairMatches(a, b, rule)) continue;
        const rank = SEVERITY_RANK[rule.severity];
        if (!chosen || rank > chosen.rank) chosen = { rule, rank };
      }
...
export function maxSeverity(items: { severity: SeverityLevel }[]): SeverityLevel | null {
