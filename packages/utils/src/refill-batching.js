...
exports.planRefillBatches = planRefillBatches;
...
function clampToWindow(date, earliest, latest) {
 ...
 */
function planRefillBatches(candidates, options = {}) {
    const { preferredPickupDow = 5, maxCopayCentsPerBatch = Number.POSITIVE_INFINITY, windowSlackDays = 3, now = new Date(), } = options;
    ...
    for (const [pharmacyId, items] of byPharmacy) {
        ...
        for (const it of items) {
            ...
            if (current &&
                ...
                if (it.pickup.getTime() > current.pickup.getTime())
                    current.pickup = it.pickup;
            }
            else {
                if (current) {
...
function toBatch(pharmacyId, pharmacyName, current) {
